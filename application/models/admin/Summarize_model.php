...
{	
	...
	function get_summarizebyday($day,$wday)
	{
		try{
		...
		{*/	
			/*$sql = "SELECT od.box_id AS id,b.name,SUM(od.qty) AS total
					...
					GROUP BY ad.item_id";*/
			//Added by Ei MOn Kyaw on 18 Aug 2016

			if($day >= date('Y-m-d') )
			{
				$now = time();
				$datediff = strtotime($day) - $now;
				$daydiff = floor($datediff/(60*60*24));
				$addday = floor($daydiff/7); 
                if($addday < 0)
                    $addday = 0;
				$sql1 = "SELECT od.box_id AS id,b.name,SUM(od.qty) AS total
				 ,SUM(od.qty) AS b_total, 0 AS i_total
					FROM `fr_order_details` AS od 
					INNER JOIN `fr_order` AS o ON o.order_id = od.order_id
					INNER JOIN `fr_box` AS b ON b.box_id = od.box_id 
					INNER JOIN `fr_delivery_info` AS di ON di.order_id = od.order_id
					INNER JOIN `fr_addresses` AS a ON a.address_id = di.address_id 
					INNER JOIN `fr_township_detail` AS ts ON ts.tspdetail_id = a.township_id
					WHERE o.order_status = 'On-going'  AND '".$day."' > order_date + INTERVAL 3 DAY AND o.week_num > o.week_status + ".$addday." AND ts.day_id = ".$wday."
					GROUP BY od.box_id 
					UNION ALL
					SELECT ad.item_id AS id,CONCAT(a.name,' (',a.net_weight,')') AS `name`,SUM(item_qty) AS total 
					,0 AS b_total, SUM(item_qty) AS i_total
					FROM `fr_additional_order` AS ad 
					INNER JOIN `fr_order` AS o ON o.order_id = ad.order_id
					INNER JOIN `fr_delivery_info` AS d ON d.order_id = ad.order_id 
					INNER JOIN `fr_addresses` AS ar ON ar.address_id = d.address_id 
					INNER JOIN `fr_township_detail` AS ts ON ts.tspdetail_id = ar.township_id
					INNER JOIN `fr_additional_items` AS a ON a.item_id = ad.item_id 
					WHERE o.order_status = 'On-going'  AND '".$day."' > order_date + INTERVAL 3 DAY AND o.week_num > o.week_status + ".$addday." AND ts.day_id = ".$wday." 
					AND o.additional_item_status = 'YES' AND (item_subscription = 'YES' OR (item_subscription = 'NO' && o.additional_item_status = 'YES' && 
					 (SELECT COUNT(*) FROM `fr_delivery_log` l WHERE l.delivery_date IS NOT NULL AND l.order_id = o.order_id) <1 )) 
					GROUP BY ad.item_id";

				$sql2 = "SELECT -- group_concat(o.order_id), 
COUNT(*) AS total_orders FROM fr_order o 
INNER JOIN `fr_delivery_info` AS di ON di.order_id = o.order_id INNER JOIN `fr_addresses` AS a ON a.address_id = di.address_id
INNER JOIN `fr_township_detail` AS ts ON ts.tspdetail_id = a.township_id
WHERE o.order_status = 'On-going'  AND '".$day."' > order_date + INTERVAL 3 DAY AND o.week_num > o.week_status + ".$addday." AND ts.day_id = ".$wday." ";
		   }
			else
			{
				$sql1 = "SELECT od.box_id AS id,b.name,SUM(od.qty) AS total
					...
					INNER JOIN `fr_additional_items` AS a ON a.item_id = ad.item_id 
					WHERE dl.order_date = '".$day."' AND o.order_status = 'On-going'  
					 AND o.additional_item_status = 'YES' AND (item_subscription = 'YES' OR (item_subscription = 'NO' AND o.additional_item_status = 'YES' AND 
					 (SELECT COUNT(*) FROM `fr_delivery_log` l WHERE l.delivery_date IS NOT NULL AND l.order_id = o.order_id) <1 )) 
					GROUP BY ad.item_id";

				$sql2 = "SELECT -- group_concat(o.order_id), 
COUNT(*) AS total_orders FROM fr_order o 
					INNER JOIN `fr_delivery_log` AS dl ON dl.order_id = o.order_id  
					WHERE dl.order_date = '".$day."' AND o.order_status = 'On-going' ";
			...
			if($query->num_rows() > 0)
				return array('data'=>$query->result(), 'count' => $query2->row()->total_orders);
			else
				return false;
		}catch(Exception $ex)
		{
	...
	function get_detailssummarizebyday($day,$wday)
	{
		try{
		$table = $this->my_data->database_table();
		/*if($day >= date('Y-m-d'))
		{*/
			...
			if($day >= date('Y-m-d') )
			{
                ...
                if($addday < 0)
                    $addday = 0;
				$sql = "SELECT b.name,IF(o.other_info = '','Default',o.other_info) AS info,SUM(od.qty) AS total, GROUP_CONCAT(o.`order_ref`) AS orders 
					FROM `fr_order_details` AS od 
					INNER JOIN `fr_order` AS o ON o.order_id = od.order_id 
					INNER JOIN `fr_delivery_info` AS d ON d.order_id = od.order_id 
					INNER JOIN `fr_box` AS b ON b.box_id = od.box_id 
